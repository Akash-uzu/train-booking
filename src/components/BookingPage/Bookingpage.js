...
const Bookingpage = () => {
  ...
  localStorage.setItem("totalseats", a ) /// pass it to seat rendering component
  const pList=(e)=>{
    if(name && age && sex && counter < tickets){

      //setPassengers((prev)=> {return [...prev,{name:name,age:age,sex:sex, bookedAgent:bookedAgent}]} )
      setPassengers(prev=>[...prev,{name:name,age:age,sex:sex, bookedAgent:bookedAgent}])
  ...
  return (
    <div className='main-booking'>
        ...
        <div className='p-main-list'>
            {passengers && passengers.map((pass)=><div key={Math.random()*10} className='p-list'>
              <h3>Passenger</h3>
              <p><span>Name:</span> {pass.name}</p>
              <p><span>Age:</span> {pass.age}</p>
              <p><span>Sex:</span> {pass.sex}</p>
       ...
       <div className='seats'>
            ...
            <div className='render-seats'>
                <Passenger age = {age} sex= {sex} bookedAgent = {bookedAgent}/>
              </div>
